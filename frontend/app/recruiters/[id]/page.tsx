    ...
    <div className="flex-1 p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        <header className="border-b border-border pb-6 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-light tracking-wide uppercase text-text">RECRUITER DOSSIER</h1>
            <p className="text-text-muted font-mono text-xs mt-2">ID: {recruiter.id}</p>
          </div>
        ...
        <div className="panel p-0 border-border">
          <h2 className="tech-label text-text-muted border-b border-border p-6 mb-0">IDENTITY & CONTACT CHANNELS</h2>
          <div className="p-6 space-y-4 font-mono text-sm text-text">
            <div className="grid grid-cols-3 gap-2 pb-4 border-b border-border/50">
              <span className="text-text-muted uppercase tracking-widest text-xs">Name:</span>
              <span className="col-span-2 text-text">{recruiter.name || "Unknown Recruiter"}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 pb-4 border-b border-border/50">
              <span className="text-text-muted uppercase tracking-widest text-xs">Email:</span>
              <span className="col-span-2 text-text">{recruiter.email || "No email recorded"}</span>
            </div>
            {recruiter.phone && (
              <div className="grid grid-cols-3 gap-2 pb-4 border-b border-border/50">
                <span className="text-text-muted uppercase tracking-widest text-xs">Phone:</span>
                <span className="col-span-2 text-text">{recruiter.phone}</span>
              </div>
            )}
            <div className="grid grid-cols-3 gap-2 pb-4 border-b border-border/50">
              <span className="text-text-muted uppercase tracking-widest text-xs">Associated Company:</span>
              <span className="col-span-2 text-text">
                {recruiter.company ? (
                  <Link href={`/companies/${recruiter.company.id}`} className="hover:text-text hover:underline">
        ...
        <div className="panel p-6 border-border">
          ...
          <p className="text-text font-mono text-sm mb-6">{recruiter.activity_summary}</p>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="p-3 bg-surface-elevated border-l-2 border-slate-500">
              <span className="block text-xs text-text-muted mb-1">Total Opportunities</span>
              <span className="text-xl font-light text-text">{recruiter.historical_opportunities_count}</span>
            </div>
            <div className="p-3 bg-surface-elevated border-l-2 border-risk-critical/50">
              <span className="block text-xs text-text-muted mb-1">High-Risk Count</span>
          ...
          {recruiter.recurring_risk_signals?.length > 0 && (
            <div className="text-sm font-mono text-text mb-6">
              <span className="text-text-muted block mb-2">Recurring risk signals from this recruiter:</span>
